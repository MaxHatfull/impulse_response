RSpec.describe SoundHit do
  describe "#initialize" do
    ...
    it "stores the raycast_hit" do
      sound_hit = SoundHit.new(raycast_hit: raycast_hit, travel_distance: 10, total_bounces: 0)

      expect(sound_hit.raycast_hit).to eq(raycast_hit)
    end

    it "stores the travel_distance" do
      sound_hit = SoundHit.new(raycast_hit: raycast_hit, travel_distance: 15.5, total_bounces: 0)

      expect(sound_hit.travel_distance).to eq(15.5)
    end

    it "stores the total_bounces" do
      sound_hit = SoundHit.new(raycast_hit: raycast_hit, travel_distance: 10, total_bounces: 3)
